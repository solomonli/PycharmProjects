...
# great news! both blocks are working!
...
def word_count_dict(filename):
    ...
    # which can be a handy way to deal with the text all at once, such as with regular expressions we'll see later.
...
def print_words(filename):
    count_dict = word_count_dict(filename)
...
def print_top(filename):
    ...
    sorted_list_frequency = sorted(count_dict.items(), key=get_count, reverse=True)
    # items() returns a list of (key, value) tuples,
    # which is the most efficient way to examine all the key value data in the dictionary

    for item in sorted_list_frequency[:20]:
        print(item[0], item[1])
